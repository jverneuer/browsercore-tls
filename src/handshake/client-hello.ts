...
export function buildClientHello(
    ...
    const extensions = buildClientHelloExtensions(config, keyPairs, provider, greaseValue);
 ...
 *
 * When GREASE is enabled, a GREASE extension (type matching the per-connection
 * sentinel, empty body) is prepended ahead of the profile's order, matching
 * real-browser behavior (RFC 8701).
 */
function buildClientHelloExtensions(
    ...
    for (const type of order) {
        const body = encodeExtensionBody(type, config, keyPairs, provider, greaseValue);
...
function encodeExtensionBody(
    ...
    switch (type) {
        ...
        case ExtensionType.KEY_SHARE:
            return encodeKeyShareClient(config, keyPairs, provider, greaseValue);
 ...
 *
 * When GREASE is enabled, a GREASE key-share entry (group matching the
 * per-connection sentinel) with a random key precedes the real groups —
 * matching real-browser ClientHellos (RFC 8701).
 */
