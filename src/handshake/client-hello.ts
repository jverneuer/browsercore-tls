 ...
 */
export function buildClientHello(config: ClientHelloConfig, keyPairs: readonly KeyPair[], provider: CryptoProvider = crypto): Uint8Array {
    const random = provider.randomBytes(32);
    const sessionId = new Uint8Array(0);
    const compressionMethods = new Uint8Array([0x00]);

    const extensions = buildClientHelloExtensions(config, keyPairs, provider);
 ...
 */
function buildClientHelloExtensions(config: ClientHelloConfig, keyPairs: readonly KeyPair[], provider: CryptoProvider = crypto): Uint8Array {
    ...
    for (const type of order) {
        const body = encodeExtensionBody(type, config, keyPairs, provider);
 ...
 */
function encodeExtensionBody(type: number, config: ClientHelloConfig, keyPairs: readonly KeyPair[], provider: CryptoProvider = crypto): Uint8Array {
    switch (type) {
        ...
        case ExtensionType.KEY_SHARE:
            return encodeKeyShareClient(config, keyPairs, provider);
 ...
 */
function encodeKeyShareClient(config: ClientHelloConfig, keyPairs: readonly KeyPair[], provider: CryptoProvider = crypto): Uint8Array {
